...
export default class Db {
    ...
    async findAllEmployees() {
        return this.query(`SELECT employee.id, employee.first_name, employee.last_name, role.title, 
    ...
    // Find all roles
    async findAllRoles() {
        return this.query('SELECT role.id, role.title, role.salary, department.department_name AS department FROM role LEFT JOIN department ON role.department_id = department.id');
    }
    ...
    async findAllManagers() {
        return this.query(`SELECT DISTINCT manager.id, CONCAT(manager.first_name, ' ', manager.last_name) AS name 
       FROM employee 
    ...
    async createRole(role) {
        ...
        return this.query('INSERT INTO role (title, salary, department_id) VALUES ($1, $2, $3)', [title, salary, departmentId]);
    }
    // Remove a role by ID
    ...
    async findUtilizedBudgetByDepartment() {
        return this.query(`SELECT department.department_name AS department, COALESCE(SUM(role.salary), 0) AS utilized_budget 
       FROM department 
